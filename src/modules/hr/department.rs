...
use crate::database::{models::{Department, NewDepartment}, schema::departments, connection::DatabaseConnection};
use chrono::Utc;
...
impl DepartmentService {
    ...
    pub fn create_department(
        ...
        if existing.is_some() {
            return Err(CLIERPError::ValidationError(
                format!("Department '{}' already exists", dept_name)
            ));
        }
    ...
    pub fn get_department_by_id(&self, conn: &mut DatabaseConnection, dept_id: i32) -> CLIERPResult<Option<Department>> {
        use crate::database::schema::departments::dsl::*;
    ...
    pub fn get_department_by_name(&self, conn: &mut DatabaseConnection, dept_name: &str) -> CLIERPResult<Option<Department>> {
        use crate::database::schema::departments::dsl::*;
    ...
    pub fn update_department(
        ...
        let dept = self.get_department_by_id(conn, dept_id)?
        ...
        if let Some(ref new_name_val) = new_name {
            if new_name_val != &dept.name {
                ...
                if existing.is_some() {
                    return Err(CLIERPError::ValidationError(
                        format!("Department '{}' already exists", new_name_val)
                    ));
                }
            }
        }
    ...
    /// Delete department
    pub fn delete_department(&self, conn: &mut DatabaseConnection, dept_id: i32) -> CLIERPResult<()> {
        use crate::database::schema::departments::dsl::*;
        use crate::database::schema::employees::dsl as emp_dsl;

        // Check if department exists
        let _dept = self.get_department_by_id(conn, dept_id)?
            .ok_or_else(|| CLIERPError::NotFound(format!("Department with ID {} not found", dept_id)))?;
        ...
        if employee_count > 0 {
            return Err(CLIERPError::ValidationError(
                format!("Cannot delete department: {} employees are still assigned to it", employee_count)
            ));
        }

        diesel::delete(departments.filter(id.eq(dept_id)))
            .execute(conn)?;
    ...
    pub fn list_departments_with_employee_count(&self, conn: &mut DatabaseConnection) -> CLIERPResult<Vec<DepartmentWithEmployeeCount>> {
        use crate::database::schema::departments::dsl::*;
        use crate::database::schema::employees::dsl as emp_dsl;
        ...
        let dept_list = departments
            ...
            .load::<((i32, String, Option<String>, Option<i32>, chrono::NaiveDateTime, chrono::NaiveDateTime), Option<i64>)>(conn)?;

        let result = dept_list
            .into_iter()
            .map(|((dept_id, dept_name, dept_desc, mgr_id, created, updated), emp_count)| {
...
impl crate::utils::export::CsvSerializable for DepartmentWithEmployeeCount {
    fn to_csv_row(&self) -> Vec<String> {
        ...
        vec![
            ...
            self.department.manager_id.map(|id| id.to_string()).unwrap_or_default(),
            self.employee_count.to_string(),
            self.department.created_at.format("%Y-%m-%d %H:%M:%S").to_string(),
            self.department.updated_at.format("%Y-%m-%d %H:%M:%S").to_string(),
        ]
    }
}
