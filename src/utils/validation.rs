...
static PHONE_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"^01[0-9]-?[0-9]{4}-?[0-9]{4}$").unwrap()
});
...
pub fn validate_email(email: &str) -> CLIERPResult<()> {
    if EMAIL_REGEX.is_match(email) {
        ...
        Err(CLIERPError::Validation(format!("Invalid email address: {}", email)))
    }
}
...
pub fn validate_phone(phone: &str) -> CLIERPResult<()> {
    if PHONE_REGEX.is_match(phone) {
        ...
        Err(CLIERPError::Validation(format!("Invalid phone number: {}", phone)))
    }
}
